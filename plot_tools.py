...
reg = [r[1] for r in err[0]['regs']]

yp = np.concatenate([r.predict(xte) for r, xte in zip(reg,all_x_te)])

ypl = [mean_squared_error(r.predict(xte), yte) for r,xte,yte in zip(reg,all_x_te,all_y_te)]


print(err[0]['error_test'])
print(mean_squared_error(yp, all_y_te2))
print(mean_squared_error(yp, all_y_te2) - err[0]['error_test'])
